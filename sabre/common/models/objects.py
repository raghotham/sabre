...
from typing import Any, Awaitable, Callable, Optional, OrderedDict, Type, TypedDict, TypeVar, Union, cast
...
class TokenPriceCalculator:
    ...
    def input_price(self, model: str, executor: Optional[str] = None) -> float:
        return self.get(model, "input_cost_per_token", executor) or 0.0

    def output_price(self, model: str, executor: Optional[str] = None) -> float:
    ...
    def max_input_tokens(
        ...
        max_input_tokens = (
            self.get(model, "max_input_tokens", executor)
            or default
            or self.__absolute_default(executor, "max_input_tokens")
        )

        if not max_input_tokens:
            raise ValueError(
                f"max_input_tokens not found for model {model} and executor {executor} and no default provided."
            )
    ...
    def max_output_tokens(
        ...
        max_output_tokens = (
            self.get(model, "max_output_tokens", executor)
            or default
            or self.__absolute_default(executor, "max_output_tokens")
        )

        if not max_output_tokens:
            raise ValueError(
                f"max_output_tokens not found for model {model} and executor {executor} and no default provided."
            )
...
class TokenPerf:
    def __init__(
        ...
        log_file: str = Container.get_config_variable(
            "profiling_file", "LLMVM_PROFILING_FILE", default="~/.local/share/llmvm/trace.log"
        ),
        request_id: str = "",
        total_tokens: int = 0,
    ):
    ...
    def debug(self):
        if self.enabled:
            ...
            sys.stderr.write("\n")
            logging.debug(
                f"ttft: {res['ttft']:.2f} ttlt: {res['ttlt']:.2f} completion_time: {res['completion_time']:.2f}"
            )
            logging.debug(
                f"prompt_len: {res['prompt_len']} completion_len: {res['completion_len']} model: {res['model']}"
            )
            logging.debug(
                f"p_tok_sec: {res['p_tok_sec']:.2f} s_tok_sec: {res['s_tok_sec']:.2f} stop_reason: {res['stop_reason']}"
            )
        ...
        if self.enabled:
            ...
            if not os.path.exists(os.path.expanduser(self.log_file)):
                with open(os.path.expanduser(self.log_file), "w") as f:
                    f.write(
                        "name,executor,model,ttlt,ttft,prompt_tokens,completion_time,prompt_len,completion_len,p_tok_sec,s_tok_sec,p_cost,s_cost,request_id,stop_reason,stop_token,total_tokens,ticks\n"
                    )
...
class Content(AstNode):
    ...
    def from_json(cls, data: dict) -> "Content":
        ...
        if content_type == "text":
            ...
            return PdfContent(base64.b64decode(sequence), url)
...
class SearchResult(TextContent):
    def __init__(self, url: str, title: str, snippet: str, engine: str):
        ...
        super().__init__(
            url=url,
            sequence=f"SearchResult(url={self.url}, title={self.title}, snippet={self.snippet}, engine={self.engine})",
        )
...
class YelpResult(TextContent):
    def __init__(self, title: str, link: str, neighborhood: str, snippet: str, reviews: str):
        ...
        super().__init__(
            url=link,
            sequence=f"YelpResult(title={self.title}, link={self.link}, neighborhood={self.neighborhood}, snippet={self.snippet}, reviews={self.reviews})",
        )
...
class HackerNewsResult(TextContent):
    def __init__(self, title: str, url: str, author: str, comment_text: str, created_at: str):
        ...
        super().__init__(
            url=url,
            sequence=f"HackerNewsResult(title={self.title}, url={self.url}, author={self.author}, comment_text={self.comment_text}, created_at={self.created_at})",
        )
    ...
    def max_input_tokens(
        ...
        if model:
            return TokenPriceCalculator().max_input_tokens(
                model=model, default=self.default_max_input_len, executor=self.name()
            )
        else:
    ...
    def max_output_tokens(
        ...
        if model:
            return TokenPriceCalculator().max_output_tokens(
                model=model, default=self.default_max_output_len, executor=self.name()
            )
        else:
...
class HelpersExecutionStartNode(AstNode):
    ...
    def __repr__(self):
        return (
            f"HelpersExecutionStartNode(helper={self.helper_name}, round_id={self.round_id}, depth={self.depth_level})"
        )
...
class ExecutionSummaryNode(AstNode):
    ...
    def generate_tree_view(self) -> str:
        ...
        lines.append(
            f"{self.total_rounds} rounds, {self.total_helpers} helpers, {self.total_duration:.2f}s{model_str} | {helper_str}"
        )
        lines.append(f"{'=' * 70}\n")
...
class FunctionCallMeta(Call):
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(
            f"'self._result isn't set, and {self.__class__.__name__}' object has no attribute '{name}'"
        )
...
class PandasMeta(Call):
    ...
    def __str__(self):
        ...
        if self.df is not None:
            ...
            str_acc += "\n"
            str_acc += 'call "to_string()" to get the entire DataFrame as a string\n'
            return str_acc
        else:
            return "[]"
