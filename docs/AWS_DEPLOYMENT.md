...
**IMPORTANT**: Complete Part 2 (App Runner deployment) first to get the server URL.

Amplify requires one-time setup via AWS CLI:

### Get the App Runner URL

First, retrieve the auto-generated URL from your App Runner service:

```bash
# Get the App Runner service URL (automatically generated by AWS)
export SERVER_URL=$(aws apprunner list-services --region us-east-1 \
  | jq -r '.ServiceSummaryList[] | select(.ServiceName=="sabre-server") | .ServiceUrl')

echo "Server URL: https://$SERVER_URL"
# Example output: https://abc123def456.us-east-1.awsapprunner.com
```
...
# 4. Set environment variable with your App Runner URL
aws amplify update-branch \
  --app-id $AMPLIFY_APP_ID \
  --branch-name main \
  --environment-variables NEXT_PUBLIC_API_URL=https://$SERVER_URL \
  --region us-east-1
